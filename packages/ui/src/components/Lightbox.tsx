...
export function Lightbox({ src, photos, index = 0, alt, onClose }: Props) {
  ...
  return createPortal(
    ...
    >
      ...
      <div className="fixed start-4 top-4 flex gap-2" onClick={(e) => e.stopPropagation()}>
        <button type="button" onClick={() => zoomBy(0.4)} aria-label="تكبير" className="h-9 w-9 rounded-full bg-soft/20 text-xl leading-none text-soft hover:bg-soft/30">＋</button>
        <button type="button" onClick={() => zoomBy(-0.4)} aria-label="تصغير" className="h-9 w-9 rounded-full bg-soft/20 text-xl leading-none text-soft hover:bg-soft/30">－</button>
      </div>

      {/* prev / next + counter */}
      {many && (
        <div className="fixed inset-x-0 bottom-4 flex items-center justify-center gap-3" onClick={(e) => e.stopPropagation()}>
          <button type="button" onClick={() => go(1)} className="rounded-full bg-soft/20 px-4 py-1.5 text-sm text-soft hover:bg-soft/30">التالي</button>
          ...
          <button type="button" onClick={() => go(-1)} className="rounded-full bg-soft/20 px-4 py-1.5 text-sm text-soft hover:bg-soft/30">السابق</button>
        </div>
      )}
    </div>,
    document.body,
