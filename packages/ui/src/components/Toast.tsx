...
export function Toaster() {
  ...
  useEffect(() => {
    ...
    function on(e: Event) {
      ...
      // Errors linger longer so slow readers can catch them.
      setTimeout(() => setItems((p) => p.filter((x) => x.id !== id)), d.kind === 'error' ? 4000 : 2600);
    }
  ...
  return (
    <div className="pointer-events-none fixed inset-x-0 bottom-4 z-[200] flex flex-col items-center gap-2" aria-live="polite">
      {items.map((it) => (
        <div key={it.id} className={`flex items-center gap-2 rounded-xl px-5 py-3 text-base font-bold text-white shadow-lg ${it.kind === 'error' ? 'bg-red-600' : 'bg-green-600'}`}>
          <span aria-hidden>{it.kind === 'error' ? '✕' : '✓'}</span>
          {it.text}
        </div>
      ))}
    </div>
