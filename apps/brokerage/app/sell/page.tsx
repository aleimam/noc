...
export default async function SellPage() {
  const [content, cityRows, districtRows, hoodRows] = await Promise.all([
    ...
    prisma.district.findMany({ where: { isActive: true }, orderBy: [{ order: 'asc' }, { nameAr: 'asc' }], select: { id: true, nameAr: true } }),
    prisma.neighborhood.findMany({ where: { isActive: true }, orderBy: [{ order: 'asc' }, { nameAr: 'asc' }], select: { id: true, nameAr: true, districtId: true } }),
  ]);
