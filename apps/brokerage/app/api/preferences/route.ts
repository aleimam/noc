...
export async function GET() {
  ...
  if (session?.user?.type !== 'CUSTOMER') {
    // Not a customer: no stored preference, but not an error (avoids console noise).
...
export async function POST(req: NextRequest) {
  const session = await auth();
  if (session?.user?.type !== 'CUSTOMER') {
    // Anonymous visitors persist their theme/locale via cookie on the client only;
    // there's nothing to save server-side, so respond 200 (no console error).
    return NextResponse.json({ ok: true, saved: false });
  }
