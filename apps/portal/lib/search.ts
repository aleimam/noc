...
// Feeds the instant search box. Two sources: a CORPUS of real vocabulary (property types +
// district/neighborhood names, cached ~5 min since it barely changes) matched by prefix/substring,
// and TRENDING (top recent successful queries from SearchLog) shown when the box is empty. Suggestions
// stay cheap so the typeahead is snappy.
...
export async function getSearchSuggestions(qRaw: string, opts: { site: SearchSite; surface: SearchSurface }): Promise<Suggestion[]> {
  ...
  // Prefix matches first (stronger signal), then substring — dedup by normalized text.
