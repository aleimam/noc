...
export async function getSiteConfig(): Promise<SiteConfig> {
  try {
    const rows = await prisma.setting.findMany({
      where: { key: { in: [SITE_KEYS.mobileMenu, SITE_KEYS.slogan, SITE_KEYS.sloganEn, SITE_KEYS.copyrightNewobour, SITE_KEYS.copyrightNewobourEn, SITE_KEYS.whatsappHelp] } },
    });
    ...
    return { mobileMenuMode: 'full', slogan: DEFAULT_SLOGAN_NEWOBOUR, sloganEn: DEFAULT_SLOGAN_NEWOBOUR_EN, copyright: DEFAULT_COPYRIGHT_NEWOBOUR, copyrightEn: DEFAULT_COPYRIGHT_NEWOBOUR_EN, whatsappHelp: '' };
  }
}
