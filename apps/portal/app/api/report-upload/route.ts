...
// Public photo upload for the missed-sheet report form (visitors aren't logged in).
// Images only, bytes sniffed (never trust the client type), per-IP rate limited.
// The attachment stays a draft (ownerType null) until the report action claims it.
...
export async function POST(req: NextRequest) {
  if (!rateLimit(`report-up:${clientIp(req.headers)}`, 10, 60 * 1000)) {
  ...
  const attachment = await prisma.attachment.create({
    data: { filename, originalName: file.name || filename, path: `/uploads/${sub}/${filename}`, mime, size: buf.length },
  });
