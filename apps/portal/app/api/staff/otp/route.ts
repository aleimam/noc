...
/** Send a login OTP to a STAFF account resolved by identifier (email/username/phone).
 *  `channel` picks delivery: 'email' → the account email, otherwise → SMS to the phone. */
export async function POST(req: NextRequest) {
  if (!rateLimit(`sotp:${clientIp(req.headers)}`, 5, 10 * 60 * 1000)) {
