...
// New Obour marketplace. No session join: sessionId is client-side only, so we attribute by
// (site + same normalized query / same opened listing + recency). Public signal, no auth.
// ALWAYS returns { ok:true } — a failed beacon must never surface an error to the visitor.
