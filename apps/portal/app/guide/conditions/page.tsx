          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {rows.map((r) => (
              <Link key={r.id} href={`/guide/conditions/${r.slug}`} className="rounded-2xl border border-ink-200 bg-white p-5 transition hover:border-gold hover:shadow-md dark:bg-navy-900">
                <div className="text-xl font-bold text-navy-800 dark:text-soft">{locale === 'en' ? r.unitLabelEn : r.unitLabelAr}</div>
                <div className="mt-1 text-sm text-ink-600">{locale === 'en' ? r.titleEn : r.titleAr}</div>
              </Link>
            ))}
          </div>
        )}
