...
import { AdvantagesEditor, AreaMapEditor, AdjacencyEditor, UpdatesEditor, InheritedUpdates, CustomPhotosEditor, AreasEditor, GeoBasics } from '../../../GeoContentEditors';
...
export default async function NeighborhoodEdit({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [advantages, updates, maps, followers, adjacency, others, inherited, amenityOptions, attachedAmenities, districts] = await Promise.all([
    ...
    <div className="space-y-6">
      ...
      <section className="space-y-2">
        ...
        <GeoBasics
          ...
          id={id}
          locale={locale}
          initial={{ nameAr: n.nameAr, nameEn: n.nameEn, order: n.order, isActive: n.isActive, parentId: n.districtId }}
          parents={districts.map((d) => ({ id: d.id, name: L(d.nameAr, d.nameEn) }))}
          tags={{ buildingTypes: (n.buildingTypes as string[] | null) ?? [], mainRoads: (n.mainRoads as string[] | null) ?? [] }}
        />
      </section>
      ...
      <div className="flex justify-end border-t border-graphite/15 pt-4"><DoneButton href="/admin/lands/neighborhoods" /></div>
    </div>
  );
}
