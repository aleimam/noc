...
// The watermark page is split by brand (owner request). Listing photos + maps carry the
// Al Sawarey brand (the listing's channel); everything else is New Obour.
...
export function WatermarkClient({ initial, contacts }: { initial: StampSettings; contacts: Contact[] }) {
    ...
    return (
      <section key={cat} className={`space-y-3 rounded-lg border border-graphite/15 p-4 ${!s.global ? 'opacity-60' : ''}`}>
        ...
        <label className="flex items-center gap-2 text-sm">
          <input type="checkbox" checked={c.logoEnabled} onChange={(e) => patch(cat, { logoEnabled: e.target.checked })} /> ختم الشعار في الزاوية (علامة مائية)
        </label>
        <div>
          <div className="mb-1 text-sm opacity-70">شعار مخصّص (اختياري — الافتراضي شعار العلامة التجارية للقسم)</div>
        ...
        <label className="flex items-center gap-2 text-sm">
          <input type="checkbox" checked={c.footerEnabled} onChange={(e) => patch(cat, { footerEnabled: e.target.checked })} /> شريط تذييل ببيانات التواصل والأيقونات
        </label>
        <p className="text-xs opacity-60">يستخدم التذييل بيانات التواصل المُدارة أعلى القسم. الحقول التالية احتياطية تُستخدم فقط عند عدم وجود بيانات تواصل.</p>
        <div className="grid gap-3 sm:grid-cols-2">
          <label className="text-sm">نص احتياطي (سطر ١)<input value={c.footerLine1} onChange={(e) => patch(cat, { footerLine1: e.target.value })} className="mt-1 w-full rounded-md border border-graphite/20 bg-transparent px-3 py-2 text-sm" placeholder="01040810000 · WhatsApp" /></label>
          <label className="text-sm">نص احتياطي (سطر ٢)<input dir="ltr" value={c.footerLine2} onChange={(e) => patch(cat, { footerLine2: e.target.value })} className="mt-1 w-full rounded-md border border-graphite/20 bg-transparent px-3 py-2 text-sm" placeholder="alsawarey.com" /></label>
        </div>

        {cat === 'rationing-scan' ? (
          <p className="text-xs text-primary/70">تُختم كشوف التقنين عند العرض مباشرةً حسب هذه الإعدادات — لا حاجة لإعادة الختم.</p>
        ) : bakeable ? (
